...
namespace Player.PlayerStates.SubStates
{
    public class PlayerInAirState : PlayerState
    {
        ...
        protected override void DoChecks() {
            ...
            // Save the X and Y axis of the Player when we detect a ledge 
        ...
        public override void LogicUpdate() {
            ...
            // Change to Ledge Climb State if he has the Ledge of Wall 
            else if (_isTouchingWall && !_isTouchingLedge)
                StateMachine.ChangeState(Player.LedgeClimbState);
            
            // Change to Wall Jump State depends if
            // there is jumpInput
            // he's touching either the wall on the back or in the front
            ...
            else if (_jumpInput && (_isTouchingWall || _isTouchingWallBack || _wallJumpCoyoteTimer))
            {
                ...
                // Set the direction for the next wall jump
                _isTouchingWall = CollisionSenses.WallFront;
            ...
            // Change to WallSlide state, if he's touching a wall,
        ...
        // Cut the jump if the player stops holding the jump button
        private void CheckJumpMultiplier()
        {
            // If the player is jumping
            if (_isJumping)
            {
                // the jump input suddenly stops
                if (_jumpInputStop)
                {
        ...
        private void CheckCoyoteTime()
        {
            ...
            // And Time.time > (time that the player enters the InAirState + value of CoyoteTime)
            // which mean the coyote time is over
            if (_coyoteTimer && Time.time > StartTime + PlayerData.coyoteTime)
            {
                // Stop the coyote timer and decrease the number of jumps
                StopCoyoteTimer();
